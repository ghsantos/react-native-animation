...
export default class KittenCards extends Component {
  ...
  transitionNext = () => {
    Animated.parallel([
      ...
      this.setState(
        prevState => ({ items: prevState.items.slice(1) }),
        () => {
        ...
        <View style={styles.top}>
          {
            this.state.items.slice(0, 2).reverse().map(({ image, id, text }, index, items) => {
              ...
              const cardStyles = isLastItem ? animatedCardStyles : {};
              const imageStyles = isLastItem ? animatedImageStyles : {};
              ...
              return (
                <Animated.View
                  key={id}
                  style={[styles.card, cardStyles, nextStyle]}
                  {...panHandlers}
                >
                  <Animated.Image
                    source={image}
